...
def features_classification(classifier_name, feature_name, classification_type):

    print("Evaluating for {} classification with {} features and {} algorithm\n".format(classification_type, feature_name, classifier_name))
    ...
    low_confidence_window = 0.2
    label_name = 'Label' # name of the "label" column
    malware_dataset = "./data/malware_dataset.csv"
    benign_dataset = "./data/benign_dataset.csv" 
    benign_df_csv = pd.read_csv(benign_dataset,sep=";")
    malicious_df_csv = pd.read_csv(malware_dataset,sep=";")
    features = ['glcm', 'hog', 'lbp']
    ...
    def get_active_function(classification_type):
        if classification_type == "binary":
            return active_learning_binary
        elif classification_type == "multiclass":
            return active_learning_multiclass
        else:
            raise ValueError("Invalid train_type")
    ...
    #algorithms = {
     #   'svm' : svm.SVC(kernel='linear', probability=True),
      #  'knn' : KNeighborsClassifier(n_neighbors=20, p=2),
       # 'rf' : RandomForestClassifier(n_estimators=50, criterion='gini', max_depth=10, min_samples_split=2, 
       ...
       # 'gb' : HistGradientBoostingClassifier(max_depth=3, max_iter=80, learning_rate=0.01, random_state=42, min_samples_leaf=1, early_stopping=True)

    #}
    algorithms = ['rf','knn', 'svm', 'gb']
    ...
    malicious_df2 = pd.DataFrame()
    if feature_name == 'all':
        for f in features:
            feature_list_mal = extract_features_from_dataframe(malicious_df_csv, f)
            feature_list_ben = extract_features_from_dataframe(benign_df_csv, f)
            n_features = n_features + len(feature_list_ben[0])
            mal_df = pd.DataFrame(feature_list_mal, columns=[f'{f}_{i}' for i in range(len(feature_list_mal[0]))])
            ben_df = pd.DataFrame(feature_list_ben, columns=[f'{f}_{i}' for i in range(len(feature_list_ben[0]))])
            benign_df = pd.concat([benign_df,ben_df], axis=1)
            malicious_df2 = pd.concat([malicious_df2,mal_df], axis=1)
       
    else:
        feature_list_mal = extract_features_from_dataframe(malicious_df_csv, feature_name)
        feature_list_ben = extract_features_from_dataframe(benign_df_csv, feature_name)
        benign_df = pd.DataFrame(feature_list_ben, columns=[f'{feature_name}_{i}' for i in range(len(feature_list_ben[0]))])
        malicious_df2 = pd.DataFrame(feature_list_mal, columns=[f'{feature_name}_{i}' for i in range(len(feature_list_mal[0]))])
       
        n_features = len(feature_list_ben[0])
    
    
    malicious_df2['Label'] = malicious_df_csv['Label']  
    benign_df['Label'] = benign_df_csv['Label']  
    ...
    #dataset = pd.concat([malicious_df2, benign_df], axis =0)
    #dataset = dataset.reset_index(drop=True)
    #print(dataset)
    #X_train = dataset.iloc[:, :len(feature_list_ben[0])]
    #y_train = dataset['Label']
    #print(len(y_train))
    #smote = SMOTE()
    #X_train_resampled, y_train_resampled = smote.fit_resample(X_train, y_train)
    #print("AFTER ", len(y_train_resampled))

    #combined_data = pd.concat([X_train_resampled, y_train_resampled], axis=1)

    
    # Define a mapping from class labels to integers (1 to 25)
    class_label_mapping = {
        "Other": 0,
        "Adposhel": 1,
        "Agent": 2,
    ...
    #malicious_df2 = combined_data[combined_data['Label'].between(1,25)]
    #benign_df = combined_data[combined_data['Label'] == 0]
...
# the script is executed for each malicious family (in the case a dataset contains multiple "attacks"; otherwise, it will be run just once)
    ...
    for index, family in enumerate(malicious_families):
        ...
        begin = time.time()
        print("\nBenign:{}\tMalicious:{}\t({})".format(len(benign_df), len(malicious_df), family))
        for Cm in Cm_list:
            for Lm in Lm_list:
                ...
                # We create a new F
                    ...
                    print("Size of F (test data): {} ben {} mal ({} tot)\n".format(len(benign_F_df), len(malicious_F_df), len(F_df)))
                    ...
                    for run in range(n):
                        print("\trun {} of {} (for reiterate: {} on {})\n".format(run, n, reiterate, k))
                        Lb = int(Lm * Cm)
                        Lcost = Lb + Lm
                        if classification_type == 'binary':
                        ...
                        for algorithm in selected_algorithms:
                            if algorithm == 'rf':
                                base_clf = RandomForestClassifier(n_estimators=50, criterion='gini', max_depth=10, min_samples_split=2, 
                                min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features='sqrt', max_leaf_nodes=None, 
                                min_impurity_decrease=0.0, bootstrap=True, oob_score=False, n_jobs=-2, 
                                random_state=None, verbose=0, warm_start=False, class_weight=None, ccp_alpha=0.0, max_samples=None)
                            elif algorithm == 'gb':
                            ...
                            best_pseudo_scores = np.array([])
                            bestactive_scores = np.array([])
                            print("\t\t\tSL (UpperBound). Large L: {}b {}m"
                                .format(len(benign_L_df)+len(benign_U_df), len(malicious_L_df)+len(malicious_U_df), len(UL_df)))
                            SL_clf, SL_precision, SL_recall, SL_fscore, SL_trainTime, SL_testTime, SL_Fpredictions = train_test(train_df=UL_df.iloc[:, :n_features], train_labels=UL_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels, messages=1, base_clf=base_clf)
                            
                            print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(SL_fscore, SL_trainTime))
                            ...
                            sl_clf, sl_precision, sl_recall, sl_fscore, sl_trainTime, sl_testTime, sl_Fpredictions = train_test(train_df=L_df.iloc[:, :n_features], train_labels=L_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels, messages=1, base_clf=base_clf)
                            print("\t\t\tTrainTime: {:.3f}s\n".format(sl_trainTime))
                            ...
                            sl_Upredictions = sl_clf.predict(U_df.iloc[:, :n_features])
                        

                            sl_Uprobabilities =  sl_clf.predict_proba(U_df.iloc[:, :n_features])
                            ...
                            ssl_clf, ssl_precision, ssl_recall, ssl_fscore, ssl_trainTime, ssl_testTime, ssl_Fpredictions = train_test(train_df=pseudoAll_L_df.iloc[:, :n_features], train_labels=pseudoAll_L_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels, messages=1, base_clf=base_clf)
                            ssl_trainTime = ssl_trainTime + sl_predictTime + sl_trainTime
                            print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(ssl_fscore, ssl_trainTime))
                            ...
                            print("\t\t\tpssl (high confidence pseudo labels). L: {}b {}m".
                                format(len(benign_L_df), len(malicious_L_df)))
                            pssl_clf, pssl_precision, pssl_recall, pssl_fscore, pssl_trainTime, pssl_testTime, pssl_Fpredictions = train_test(train_df=pseudoHigh_L_df.iloc[:, :n_features], train_labels=pseudoHigh_L_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels, messages=1, base_clf=base_clf)
                            pssl_trainTime = pssl_trainTime + sl_predictTime + sl_trainTime
                            print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(pssl_fscore, pssl_trainTime))
                            ...
                            start_time = time.time()
                            ssl_Umlpredictions, ssl_Umlprobabilities = ssl_clf.predict(U_midlow_df.iloc[:, :n_features]), ssl_clf.predict_proba(U_midlow_df.iloc[:,:n_features])
                            ...
                            print("\t\t\trpssl (high confidence pseudo labels, twice). L: {}b {}m".
                                format(len(benign_L_df), len(malicious_L_df)))
                            rpssl_clf, rpssl_precision, rpssl_recall, rpssl_fscore, rpssl_trainTime, rpssl_testTime, rpssl_Fpredictions = train_test(train_df=pseudoHigh_high_L_df.iloc[:, :n_features], train_labels=pseudoHigh_high_L_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels, messages=1, base_clf=base_clf)
                            rpssl_trainTime = rpssl_trainTime + pssl_trainTime
                            print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(rpssl_fscore, rpssl_trainTime))
                           ...
                           ############ ACTIVE LEARNING
                            ...
                            support_sl_clf, support_sl_precision, support_sl_recall, support_sl_fscore, support_sl_trainTime, support_sl_testTime, support_sl_Fpredictions = train_test(train_df=support_L_df.iloc[:, :n_features], train_labels=support_L_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels,  messages=0, base_clf=base_clf)
                                              

                            start_time = time.time()
                            support_sl_Upredictions, support_sl_Uprobabilities = support_sl_clf.predict(support_U_df.iloc[:, :n_features]), support_sl_clf.predict_proba(support_U_df.iloc[:,:n_features])
                            ...
                            if 'high' in al_confidences:
                                ...
                                print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(ahssl_fscore, ahssl_trainTime))
                            ...
                            if 'mid' in al_confidences:
                                ...
                                print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(aossl_fscore, aossl_trainTime))
                            ...
                            if 'low' in al_confidences:
                                ...
                                print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(alssl_fscore, alssl_trainTime))
                        ...
                        # pshs = convert_from_csv(pseudoHigh_support_L_df)                  
                            

                            
                            print("\t\t\tPseudo-Active Learning: the base size of the training set is: {}".format(len(pseudoHigh_support_L_df)))
                            support_pssl_clf, support_pssl_precision, support_pssl_recall, support_pssl_fscore, support_pssl_trainTime, support_pssl_testTime, support_pssl_Fpredictions = train_test(train_df=pseudoHigh_support_L_df.iloc[:, :n_features], train_labels=pseudoHigh_support_L_labels, 
                                                                                                test_df=F_df.iloc[:, :n_features], test_labels=F_labels,  messages=0, base_clf=base_clf)
                            ...
                            support_pssl_Umlpredictions, support_pssl_Umlprobabilities = support_pssl_clf.predict(support_U_midlow_df.iloc[:, :n_features]), support_pssl_clf.predict_proba(support_U_midlow_df.iloc[:, :n_features])
                            ...
                            #print(support_U_midlow_df['support_ssl_probability'])
                            ...
                            if 'high' in al_confidences:
                                ...
                                print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(pahssl_fscore, pahssl_trainTime))
                            ...
                            if 'mid' in al_confidences:
                                ...
                                print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(paossl_fscore, paossl_trainTime))
                            ...
                            if 'low' in al_confidences:
                                ...
                                print("\t\t\tF1: {:.5f} TrainTime: {:.3f}s\n".format(palssl_fscore, palssl_trainTime))
    ...
    for algorithm, cases in algorithm_results.items():
        ...
        filename_avg = f"f1_score_execution_time_{algorithm}_{feature_name}.png"
        ...
        filename_wilcoxon = f"wilcoxon_test_{algorithm}_{feature_name}.png"
